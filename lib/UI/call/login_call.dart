...
class CallLoginScreen extends StatefulWidget {
  const CallLoginScreen({Key? key, required this.title}) : super(key: key);
  final String title;

  @override
  State<CallLoginScreen> createState() => _LoginScreenState();
}

class _LoginScreenState extends State<CallLoginScreen>
    with WidgetsBindingObserver {
  final logger = Logger();
  TextEditingController sipUserController =
      TextEditingController(text: "adewaletolu57512");
  TextEditingController sipPasswordController =
      TextEditingController(text: "FsLopXYV");
  TextEditingController sipNameController =
      TextEditingController(text: "Gomobilez");
  TextEditingController sipNumberController =
      TextEditingController(text: "3238311266");
    ...
    if (registered) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (builder) => HomeCallScreen(
                      title: '',
                    )));
      });
    }
